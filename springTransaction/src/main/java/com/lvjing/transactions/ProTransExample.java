package com.lvjing.transactions;

import com.lvjing.beans.Book;
import com.lvjing.util.TemplateUtils;
...
public class ProTransExample {
   ...
   // 事务管理器：第一种事务编程模式
    ...
    public void updateBookByIsbn(Book book) {
        // 第一步：获取JDBC事务管理器: DataSourceTransactionManager
   ...
   // 事务模板：第二种事务编程模式
    ...
    public Book findBookByIsbn(final String isbn) {
        TransactionTemplate tt = TemplateUtils.getTransactionTemplate();
        Book book = null;
        List<Map<java.lang.String, Object>> books = (List<Map<java.lang.String, Object>>)tt.execute(new TransactionCallback<Object>() {
            public Object doInTransaction(TransactionStatus arg0) {
                JdbcTemplate template = TemplateUtils.getJdbcTemplate();
                return template.queryForList("select isbn,name,price,pubdate from books wherer isbn = '" + isbn + "'");
            }
        });
        if (books!=null) {  // 封装获取的数据
            Map<String,Object> m = books.get(0);
            book = new Book();
